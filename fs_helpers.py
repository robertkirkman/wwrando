...
def read_str(data, offset, length):
  ...
  data.seek(offset)
  string = data.read(length).decode("shift_jis", errors='ignore')
...
def read_str_until_null_character(data, offset):
  ...
  data.seek(offset)
  str = data.read(str_length).decode("shift_jis", errors='ignore')
...
def write_str(data, offset, new_string, max_length):
  ...
  new_value = new_string.encode("shift_jis", errors='ignore') + null_padding
